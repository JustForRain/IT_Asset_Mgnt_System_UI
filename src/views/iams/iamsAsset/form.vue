        ...
        <el-col :span="12" class="mb20">
          <el-form-item label="关联合同" prop="contractId">
            <el-select v-model="form.contractId" placeholder="请选择关联合同">
              <el-option v-for="item in getiamsContractOption" :key="item.id" :label="item.projectName" :value="item.id" />
            </el-select>
          </el-form-item>
        </el-col>
        <el-col :span="12" class="mb20">
...
<script setup lang="ts" name="IamsAssetDialog">
import {useDict} from '/@/hooks/dict';
import {useMessage} from "/@/hooks/message";
import {getObj, addObj, putObj} from '/@/api/iams/iamsAsset'
import {fetchList} from '/@/api/iams/iamsContract'
...
const dataRules = ref({
  ...
  contractId: [{required: true, message: '关联合同ID不能为空', trigger: 'blur'}],
...
// 初始化表单数据
const getiamsAssetData = (id: string) => {
  // 获取数据
  loading.value = true
