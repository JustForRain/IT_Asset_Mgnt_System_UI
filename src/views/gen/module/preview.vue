...
import { useGeneratorModulePreviewApi } from '/@/api/gen/module';
...
const openDialog = async (moduleName: string,description: string) => {
  await getModuleCodeFile(moduleName,description);
  visible.value = true;
};
 ...
 * @param id 需要渲染的资源 ID。
 */
const getModuleCodeFile = (moduleName: string,description: string) => {
  loading.value = true;
  fileTreeOriginal.value = [];
  useGeneratorModulePreviewApi(moduleName,description)
      .then((res: any) => {
